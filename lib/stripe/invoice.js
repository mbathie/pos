...
/**
 * Create a Stripe invoice for a company or customer transaction
 * @param {Object} params
 * @param {Object} params.transaction - Transaction document
 * @param {Object} params.company - Company document (optional if customer provided)
 * @param {Object} params.customer - Customer document (optional if company provided)
 * @param {Object} params.org - Organization document
 * @param {string} params.stripeCustomerId - Stripe customer ID for the company/customer
 * @returns {Promise<Object>} - Stripe invoice object
 */
export async function createCompanyInvoice({ transaction, company, customer, org, stripeCustomerId }) {
  console.log('📄 Creating Stripe invoice for transaction:', transaction._id);

  // Determine if this is a customer invoice
  const isCustomerInvoice = !!customer && !company;
  const payerId = isCustomerInvoice ? customer._id : company._id;
  const paymentTerms = isCustomerInvoice ? 'due_on_receipt' : (company.paymentTerms || 'due_on_receipt');
  ...
  try {
    ...
    const invoice = await stripe.invoices.create({
      ...
      days_until_due: getPaymentTermsDays(paymentTerms),
      metadata: {
        transactionId: transaction._id.toString(),
        ...(isCustomerInvoice
          ? { customerId: customer._id.toString(), type: 'customer-invoice' }
          : { companyId: company._id.toString(), type: 'company' }
        ),
        orgId: org._id.toString(),
        locationId: transaction.location?.toString() || ''
      },
      description: `Purchase from ${org.name}`,
      footer: transaction.companyPayment
