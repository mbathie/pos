...
/**
 * Get or create a Stripe customer for an individual customer (for group invoice payments)
 * @param {Object} params
 * @param {Object} params.customer - Customer document from database
 * @param {Object} params.org - Organization document with stripeAccountId
 * @returns {Promise<string>} - Stripe customer ID
 */
export async function getOrCreateCustomerStripeCustomer({ customer, org }) {
  // If customer already has a Stripe customer ID for this org, return it
  if (customer.stripeCustomerId) {
    console.log('✅ Customer already has Stripe customer:', customer.stripeCustomerId);
    return customer.stripeCustomerId;
  }

  console.log('🆕 Creating new Stripe customer for individual:', customer.name);

  // Prepare address object (only include if we have at least one field)
  const address = {};
  if (customer.address?.address1) address.line1 = customer.address.address1;
  if (customer.address?.address2) address.line2 = customer.address.address2;
  if (customer.address?.city) address.city = customer.address.city;
  if (customer.address?.state) address.state = customer.address.state;
  if (customer.address?.postcode) address.postal_code = customer.address.postcode;
