...
export async function createCashTransaction({ cart, employee, received, change }) {
  ...
  // Extract discount info from new adjustments structure
  let discountId = cleanCart.appliedDiscountId || cleanCart.discount?._id || null;
  // Convert to ObjectId if it's a string
  if (discountId && typeof discountId === 'string') {
    discountId = Types.ObjectId.createFromHexString(discountId);
  }
  
  // Calculate total discount amount from products if not available in adjustments
  let discountAmount = cleanCart.adjustments?.totalDiscountAmount || cleanCart.discountAmount || 0;
  if (!discountAmount && cleanCart.products) {
    discountAmount = cleanCart.products.reduce((total, product) => {
      return total + (product.amount?.discount || 0);
    }, 0);
  }
