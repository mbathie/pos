...
import { addToPrepaid } from '@/lib/prepaid'
...
import { Transaction, Membership, Customer, Schedule } from '@/models'
...
/**
 * Creates a company transaction (zero payment upfront, payment handled later)
 * @param {Object} params - Parameters
 * @param {Object} params.cart - Cart data
 * @param {Object} params.employee - Employee data
 ...
 * @returns {Object} - Created transaction
 */
export async function createCompanyTransaction({ cart, employee, company }) {
  const cleanCart = prepareCartForTransaction(cart);
  ...
  // Extract schedule information from cart if it's a class/course purchase
  let scheduleInfo = null;
  if (cleanCart.products?.length > 0) {
    const firstProduct = cleanCart.products[0];
    // Check if this is a class/course purchase with schedule information
    if (firstProduct.type === 'class' || firstProduct.type === 'course') {
      if (firstProduct.selectedTimes?.length > 0 && firstProduct._id) {
        // Query Schedule collection to get the actual schedule ID
        // (firstProduct._id is the product ID, not the schedule ID)
        try {
          const schedule = await Schedule.findOne({
            product: firstProduct._id,
            'locations.location': employee.selectedLocationId
          }).select('_id').lean();
          ...
          if (schedule) {
            ...
            scheduleInfo = {
              ...
              classDatetime: selectedTime.datetime
            };
            console.log('✅ Found schedule for company transaction:', {
            ...
            console.warn('⚠️ No schedule found for product:', firstProduct._id, 'at location:', employee.selectedLocationId);
          }
        } catch (error) {
          console.error('❌ Error finding schedule for company transaction:', error);
        }
      }
    }
  }
  ...
  const transaction = await Transaction.create({
    ...
    employee: employee._id,
    customer: null, // No customer assigned yet - will be assigned via waiver link
    location: employee.selectedLocationId,
    paymentMethod: "company",
    company: company._id, // Link to company making the purchase
    cart: cleanCart,
    status: "succeeded", // Mark as succeeded even though payment is deferred
    // Store company payment metadata
    companyPayment: {
      companyName: company.name,
      contactEmail: company.contactEmail,
      contactName: company.contactName,
      abn: company.abn,
      paymentDeferred: true, // Flag to indicate payment is handled separately
      waiverLinkSent: false, // Will be updated when waiver link is sent
      // Include schedule information for linking customers later
      ...(scheduleInfo && {
        scheduleId: scheduleInfo.scheduleId,
        locationId: scheduleInfo.locationId,
        classDatetime: scheduleInfo.classDatetime
      })
    }
  });
...
/**
 * Creates a customer invoice transaction (for group bookings paid by an individual customer)
 * @param {Object} params - Parameters
 * @param {Object} params.cart - Cart data
 * @param {Object} params.employee - Employee data
 * @param {Object} params.customer - Customer data (the payer, not attendees)
 * @returns {Object} - Created transaction
 */
export async function createCustomerInvoiceTransaction({ cart, employee, customer }) {
  const cleanCart = prepareCartForTransaction(cart);
      ...
      if (firstProduct.selectedTimes?.length > 0 && firstProduct._id) {
        try {
          ...
          if (schedule) {
            ...
            console.log('✅ Found schedule for customer invoice transaction:', {
              productId: firstProduct._id,
              scheduleId: schedule._id,
              locationId: employee.selectedLocationId,
              classDatetime: selectedTime.datetime
          ...
          console.error('❌ Error finding schedule for customer invoice transaction:', error);
        }
      }
    }
  }
  ...
  const transaction = await Transaction.create({
    ...
    adjustments: adjustments,
    employee: employee._id,
    customer: customer._id, // The customer paying (not attendees)
    location: employee.selectedLocationId,
    paymentMethod: "customer-invoice",
    cart: cleanCart,
    status: "succeeded", // Mark as succeeded even though payment is deferred
    // Store customer invoice payment metadata (similar structure to company payment)
    companyPayment: {
      companyName: customer.name, // Use customer name
      contactEmail: customer.email,
      contactName: customer.name,
      abn: null, // Customers don't have ABN
      paymentDeferred: true,
      waiverLinkSent: false,
      isCustomerInvoice: true, // Flag to identify this as a customer invoice
      ...(scheduleInfo && {
        scheduleId: scheduleInfo.scheduleId,
        locationId: scheduleInfo.locationId,
        classDatetime: scheduleInfo.classDatetime
  ...
  console.log('✅ Customer invoice transaction created:', {
    transactionId: transaction._id,
...
/**
 * Add placeholder customers to schedule to block spots immediately after purchase
 * This must be called AFTER handleTransactionSuccess() so the schedule exists
 */
export async function addPlaceholdersToSchedule({ transaction }) {
  console.log('🔄 Adding placeholders to schedule for transaction:', transaction._id);

  if (!transaction.companyPayment) {
    console.log('⚠️ Not a company payment, skipping placeholder creation');
    return;
  }

  // Extract info from cart (companyPayment might not have schedule info if schedule didn't exist yet)
  if (!transaction.cart?.products?.[0]) {
    console.log('⚠️ No products in cart, skipping placeholder creation');
    return;
  }

  const product = transaction.cart.products[0];

  // Check if this is a class/course purchase
  if (!['class', 'course'].includes(product.type)) {
    console.log('⚠️ Product is not a class or course, skipping placeholder creation');
    return;
  }

  // Get datetime from cart's selectedTimes (always available in cart)
  const selectedTime = product.selectedTimes?.[0];
  if (!selectedTime?.datetime) {
    console.log('⚠️ No selected time found in cart');
    return;
  }

  const locationId = transaction.location;
  const classDatetime = selectedTime.datetime;

  if (!product._id || !locationId || !classDatetime) {
    console.log('⚠️ Missing required info for placeholder creation:', {
  ...
  try {
    // Query for the schedule by product and org (it was just created by addToSchedule)
    const schedule = await Schedule.findOne({
      product: product._id,
      org: transaction.org
    });
    ...
    if (!schedule) {
      console.error('❌ Schedule not found for product:', product._id);
      return;
    }

    console.log('✅ Found schedule:', schedule._id);

    // Calculate total quantity purchased across all price tiers
    const totalQty = transaction.cart.products[0].prices.reduce((sum, p) => sum + (p.qty || 0), 0);

    console.log('📊 Total quantity to reserve:', totalQty);
    ...
    if (locationIndex === -1) {
      console.error('❌ Location not found in schedule:', locationId);
      return;
    }

    // Find the class
    const classIndex = schedule.locations[locationIndex].classes.findIndex(
      cls => new Date(cls.datetime).getTime() === new Date(classDatetime).getTime()
    );

    if (classIndex === -1) {
      console.error('❌ Class not found in schedule:', classDatetime);
      return;
    }

    // Add placeholder customer entries for each spot purchased
    ...
    schedule.locations[locationIndex].classes[classIndex].available -= totalQty;

    await schedule.save();

    console.log('✅ Added placeholder customers to schedule:', {
      scheduleId: schedule._id,
      locationIndex,
      classIndex,
      totalQty,
      ...
      newAvailable: schedule.locations[locationIndex].classes[classIndex].available
    });

  } catch (error) {
    console.error('❌ Error adding placeholder customers to schedule:', error);
...
export async function handleTransactionSuccess({ transaction, cart, employee }) {
  try {
    ...
    // Create prepaid passes
    await addToPrepaid({ cart, employee, transaction });
...
async function sendEmailReceipt({ transaction, cart, employee }) {
  try {
    // Don't send receipt for deferred payment methods (invoice, customer-invoice)
    // Receipt will be sent when the invoice is fully paid via the Stripe webhook
    const deferredMethods = ['invoice', 'customer-invoice', 'company'];
    if (deferredMethods.includes(transaction.paymentMethod) && transaction.companyPayment?.paymentDeferred) {
      console.log(`ℹ️ Deferred payment (${transaction.paymentMethod}), skipping receipt until fully paid for transaction ${transaction._id}`);
      return;
    }
    ...
    // Main transaction/cart customer
    if (transaction.customer?.email) {
      customerEmails.add(transaction.customer.email);
    } else if (cart.customer?.email) {
      customerEmails.add(cart.customer.email);
    }

    // Check all product slots for additional customer emails
    for (const product of cart.products || []) {
      // Check prices.customers (class/course/general bookings)
      for (const price of product.prices || []) {
        for (const customerEntry of price.customers || []) {
          if (customerEntry?.customer?.email) {
            customerEmails.add(customerEntry.customer.email);
          }
        }
      }
      // Check variations.prices.customers (legacy format)
      for (const variation of product.variations || []) {
        for (const price of variation.prices || []) {
          for (const customerEntry of price.customers || []) {
            if (customerEntry?.customer?.email) {
    ...
    // Send receipt to each unique customer email
    if (customerEmails.size > 0) {
      ...
      for (const email of customerEmails) {
        ...
        if (result.success) {
          console.log(`📧 Receipt email sent to ${email} for transaction ${transaction._id}`);
          console.log(`   Preview URL: ${result.previewUrl}`);
        } else {
          console.error(`❌ Failed to send receipt email to ${email}:`, result.error);
        }
      }
    } else {
