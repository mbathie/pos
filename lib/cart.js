export function calcCartTotals(products, discount = null) {
  ...
  const processedGroups = new Set() // Track which groups we've already counted

  for (const item of products) {
    // Check if this is a grouped product
    if (item.gId) {
      // Only count the group once (first product with this gId)
      if (!processedGroups.has(item.gId)) {
        processedGroups.add(item.gId)
        // groupAmount is now the full total for the entire group (not per-unit)
        const groupSubtotal = item.groupAmount || 0
        const groupTax = +(groupSubtotal * 0.10).toFixed(2)
        const groupTotal = +(groupSubtotal + groupTax).toFixed(2)

        totals.subtotal += groupSubtotal
        totals.tax += groupTax
        totals.total += groupTotal
      }
      // Skip individual product amounts for grouped products
    } else {
      // Regular product - use individual amounts
      if (item.amount) {
