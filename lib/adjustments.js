...
export async function calculateAdjustments({ cart, customer, discountCode, discountId, orgId, isManualSelection = false }) {
  ...
  // Enrich cart products with tags/category from DB if missing
  const productIds = cart.products?.map(p => p._id).filter(Boolean) || [];
  if (productIds.length > 0) {
    const dbProducts = await Product.find({ _id: { $in: productIds } }).select('tags category').lean();
    const dbMap = Object.fromEntries(dbProducts.map(p => [p._id.toString(), p]));
    for (const p of cart.products) {
      const db = dbMap[p._id?.toString()];
      if (db) {
        if (!p.tags || p.tags.length === 0) p.tags = db.tags || [];
        if (!p.category) p.category = db.category;
      }
    }
  }
    ...
    console.log(`🎯 Found discount "${discount.name}"`, {
      ...
      musts: discount.musts ? {
        ...
        categories: discount.musts.categories?.length > 0 ?
          ...
          'none',
        tags: discount.musts.tags?.length > 0 ?
          `${discount.musts.tags.length} tag(s): [${discount.musts.tags.map(t => t.toString()).join(', ')}]` :
          'none'
      ...
      adjustments: discount.adjustments?.length > 0 ?
        discount.adjustments.map((adj, idx) => ({
          ...
          tags: adj.tags?.length > 0 ?
            `${adj.tags.length} tag(s)` : 'all',
  ...
  if (discount) {
    // Check if discount requires customer
    if (discount.requireCustomer && !customer) {
      console.log(`❌ Discount "${discount.name}" requires customer identification but no customer selected`);
      updatedCart.adjustments.discountError = 'This discount requires customer identification';
      // Don't apply the discount if customer is required but not present
      discount = null;
    }
  }
    ...
    if (validationResult.isValid) {
      ...
      if (discountResult.applied) {
        ...
        updatedCart.adjustments.discounts.items.push({
          ...
          affectedProducts: discountResult.affectedProducts
        });
...
export async function findBestAutoDiscount({ cart, customer, orgId }) {
  ...
  for (const discount of discounts) {
    ...
    if (validation.isValid) {
      ...
      // Initialize originalSubtotal on test cart products (normally done in calculateAdjustments)
      for (const p of testCart.products) {
        p.originalSubtotal = p.amount?.subtotal || 0;
      }
...
async function validateDiscount(discount, customer, cart) {
  ...
  if (discount.musts) {
    ...
    const mustHaveTags = discount.musts.tags || [];

    if (mustHaveProducts.length > 0 || mustHaveCategories.length > 0 || mustHaveTags.length > 0) {
      ...
      let hasRequiredItems = cart.products.some(product => {
        ...
        const tagMatch = mustHaveTags.some(tid =>
          product.tags &&
          product.tags.length > 0 &&
          product.tags.some(productTag =>
            (productTag._id || productTag).toString() === tid.toString()
          )
        );
        return productMatch || categoryMatch || tagMatch;
      });
      
      // If not in cart and customer is provided, check for membership products, categories, or tags
      if (!hasRequiredItems && customer && (mustHaveProducts.length > 0 || mustHaveCategories.length > 0 || mustHaveTags.length > 0)) {
        console.log(`🔍 Checking customer's active memberships for must-have requirements...`);
        
        // Get membership product IDs from must-have list (both direct products and products in categories)
        const membershipProductIds = [];
        
        // Check direct product requirements
        ...
        // Check category requirements for membership products
        for (const categoryId of mustHaveCategories) {
          // Find all membership products in this category
          const membershipProducts = await Product.find({
            category: categoryId,
            type: 'membership'
          }).lean();

          for (const product of membershipProducts) {
            if (!membershipProductIds.includes(product._id.toString())) {
              membershipProductIds.push(product._id.toString());
              console.log(`  → Found membership product in required category: ${product.name} (${product._id})`);
            }
          }
        }

        // Check tag requirements for membership products
        for (const tagId of mustHaveTags) {
          // Find all membership products with this tag
          const membershipProducts = await Product.find({
            tags: tagId,
            type: 'membership'
          }).lean();
          ...
          for (const product of membershipProducts) {
            if (!membershipProductIds.includes(product._id.toString())) {
              membershipProductIds.push(product._id.toString());
              console.log(`  → Found membership product with required tag: ${product.name} (${product._id})`);
            }
          }
        }
        ...
        if (membershipProductIds.length > 0) {
          ...
          const activeMemberships = await Membership.find({
            ...
            status: { $nin: ['cancelled', 'suspended'] } // Exclude both cancelled AND suspended
          }).lean();
          ...
          if (activeMemberships.length > 0) {
            ...
            for (const membership of activeMemberships) {
              console.log(`  → Active membership: Product ${membership.product}, status ${membership.status}, expires ${membership.nextBillingDate}`);
            }
          } else {
            // Check specifically if they have suspended memberships
            const suspendedMemberships = await Membership.find({
              customer: customer._id,
              product: { $in: membershipProductIds },
              status: 'suspended'
            }).lean();
            ...
            if (suspendedMemberships.length > 0) {
              console.log(`⏸️ Customer has ${suspendedMemberships.length} suspended membership(s) - discount blocked`);
              result.isValid = false;
              result.error = 'This discount is not available while your membership is suspended';
              return result;
            }
  ...
  // Check usage limits
  if (discount.limits) {
    // Check global usage limit across all customers
    if (discount.limits.total?.usageLimit && discount.limits.total.usageLimit > 0) {
      const totalUsage = await getTotalDiscountUsage(discount._id);
      console.log(`📊 Total usage for discount "${discount.name}": ${totalUsage} / ${discount.limits.total.usageLimit}`);
      
      if (totalUsage >= discount.limits.total.usageLimit) {
        result.isValid = false;
        result.error = `This discount has reached its maximum usage limit of ${discount.limits.total.usageLimit}`;
        console.log(`❌ Discount has reached total usage limit`);
        return result;
      }
    }
    ...
    if (customer && discount.limits.perCustomer) {
      ...
      if (customerUsage.total >= discount.limits.perCustomer) {
        result.isValid = false;
        result.error = `Customer has already used this discount the maximum ${discount.limits.perCustomer} time${discount.limits.perCustomer > 1 ? 's' : ''}`;
        return result;
      }
    }
...
function getApplicableProducts(products, adjustment) {
  ...
  const hasTagRestrictions = adjustment.tags && adjustment.tags.length > 0;

  if (!hasProductRestrictions && !hasCategoryRestrictions && !hasTagRestrictions) {
  ...
  const applicable = products.filter(product => {
    ...
    // Check if product has any of the required tags
    const tagMatch = hasTagRestrictions &&
      product.tags &&
      product.tags.length > 0 &&
      adjustment.tags.some(tid =>
        product.tags.some(productTag =>
          (productTag._id || productTag).toString() === tid.toString()
        )
      );

    const isApplicable = productIdMatch || categoryMatch || tagMatch;

    if (!isApplicable && (hasProductRestrictions || hasCategoryRestrictions || hasTagRestrictions)) {
      console.log(`  → Product "${product.name}" (ID: ${product._id}, Category: ${product.category || 'none'}, Tags: ${product.tags?.map(t => t._id || t).join(', ') || 'none'}) - Not applicable`);
      ...
      if (hasTagRestrictions) {
        console.log(`    Required tag IDs: [${adjustment.tags.map(t => t.toString()).join(', ')}]`);
      }
    }
  ...
  if (discount.adjustments && discount.adjustments.length > 0) {
    ...
    for (const adjustment of discount.adjustments) {
      ...
      for (const productResult of adjResult.affectedProducts) {
        ...
        if (product) {
          // Initialize adjustments structure if missing
          if (!product.adjustments) {
            product.adjustments = { discounts: { items: [], total: 0 }, surcharges: { items: [], total: 0 } };
          }
          // Add to product's adjustments using new structure
...
/**
 * Get total discount usage across all customers
 */
async function getTotalDiscountUsage(discountId) {
  // Count all usage of this discount across all customers
  const customers = await Customer.find({
    'adjustments.discounts.discount': discountId
  }).lean();
  
  let totalUsage = 0;
  for (const customer of customers) {
    if (customer.adjustments?.discounts) {
      const usageCount = customer.adjustments.discounts.filter(d => 
  ...
  console.log(`📊 Total usage across all customers for discount ${discountId}: ${totalUsage}`);
  return totalUsage;
}
...
async function getCustomerDiscountUsage(customerId, discountId) {
  ...
  if (!customer || !customer.adjustments?.discounts) {
    return { total: 0, recent: [] };
  }
  
  const usage = customer.adjustments.discounts.filter(d => 
    d.discount && d.discount.toString() === discountId.toString()
  );
  
  console.log(`📊 Customer ${customerId} has used discount ${discountId} ${usage.length} times`);
...
async function checkFrequencyLimit(customerId, discountId, frequency) {
  ...
  const usageInPeriod = customer.adjustments.discounts.filter(d => 
    d.discount && 
    d.discount.toString() === discountId.toString() &&
    new Date(d.usedAt) >= periodStart
  );
  
  if (usageInPeriod.length >= frequency.count) {
    return `Customer can only use this discount ${frequency.count} time${frequency.count > 1 ? 's' : ''} per ${frequency.period}`;
  }
...
export function calculateCustomDiscount(cart, discountAmount) {
  ...
  // Import generateObjectId function
  const { generateObjectId } = require('@/lib/utils');
  
  // Set up adjustments structure for UI consistency - match the structure from calculateAdjustments
  updatedCart.adjustments = {
    discounts: {
      items: [{
        id: generateObjectId(), // Generate a proper ObjectId for the custom discount
        name: 'Custom Discount',
        code: null,
        type: 'amount',
        value: discountAmount,
        amount: +actualDiscountApplied.toFixed(2),
        custom: true, // Mark this as a custom discount
      ...
      total: +actualDiscountApplied.toFixed(2)
    },
    surcharges: {
      items: [],
      total: 0
