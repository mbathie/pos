  ...
  if (discount) {
    ...
    if (validationResult.isValid) {
      ...
      if (discountResult.applied) {
        ...
        // The UI reads discounts[0].totalDiscountAmount; ensure it's present alongside totalAmount
        const normalizedDiscountResult = {
          ...discountResult,
          totalDiscountAmount: +Number(discountResult.totalAmount || 0).toFixed(2)
        };
        updatedCart.adjustments.discounts.push(normalizedDiscountResult);
...
export async function findBestAutoDiscount({ cart, customer, orgId }) {
  ...
  // Get all active discounts for the org (including those with codes)
  // All discounts can be auto-applied if customer meets requirements
  ...
  console.log(`🔍 Found ${discounts.length} potential auto-applicable discounts`);
  ...
  if (discount.adjustments && discount.adjustments.length > 0) {
    ...
    for (const adjustment of discount.adjustments) {
      ...
      for (const productResult of adjResult.affectedProducts) {
        ...
        if (product) {
          product.amount.discount = (product.amount.discount || 0) + productResult.amount;
