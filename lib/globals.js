...
export const useGlobals = create(persist((set, get) => ({
  ...
  carts: [{
    ...
    customer: null // customer linked to saved cart { _id, name, email, phone }
  }],
  currentCartIndex: 0,
  ...
  loadGroupForEditing: (groupProducts, transactionId) => set((state) => {
    ...
    if (!groupProducts || !Array.isArray(groupProducts)) return state;
  ...
  saveCart: (customer) => set((state) => {
    ...
    // Mark current cart as saved with timestamp and customer
    const newCarts = [...state.carts];
    newCarts[currentIdx] = {
      ...currentCart,
      savedAt: Date.now(),
  ...
  // Current POS interface name for this device (for display in header)
  posInterfaceName: null,
  setPosInterfaceName: (name) => set({ posInterfaceName: name }),
