 ...
 */
export async function sendCompanyWaiverEmail({ company, org, waiverLink, transaction, invoiceUrl }) {
  try {
    ...
    const content = `
      ...
      <div style="background-color: #fff3cd; border-left: 4px solid #ffc107; padding: 16px; margin: 24px 0; border-radius: 4px;">
        ...
        <p style="margin: 12px 0 0; color: #856404; font-size: 13px; line-height: 20px;">
          The invoice supports partial payments. You can pay the full amount or make partial payments as needed.
        </p>
      </div>
      ` : ''}
    ...
    const emailHTML = generateEmailLayout({
      content,
      org,
      title: `Waiver Link for ${company.name}`,
      headerText: `Waiver from ${org.name}`,
      subHeaderText: `For ${company.name}`
    });
...
View and pay invoice: ${invoiceUrl}
