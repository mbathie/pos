      ...
      <table width="100%" cellpadding="0" cellspacing="0" style="padding: ${isGrouped ? '6px 0' : '12px 0'}; ${index > 0 ? 'border-top: 1px solid #e3e8ee;' : ''}">
        <tr>
          <td style="vertical-align: top;">
            ...
            ` : ''}
            ${product.validityDuration && product.validityUnit ? `
              <div style="margin-top: 6px;">
                <span style="
                  ...
                  letter-spacing: 0.3px;
                ">
                  Valid for ${product.validityDuration} ${product.validityUnit}${product.validityDuration > 1 ? 's' : ''} from purchase
                </span>
              </div>
            ` : ''}
...
export async function sendTransactionReceipt({ transaction, recipientEmail, org, paymentAmount }) {
  try {
    ...
    // Generate QR code as data URL and prepare as CID attachment
    const attachments = [];
    const hasCustomerQR = needsQRCode && customer?._id;

    if (hasCustomerQR) {
      const qrDataUrl = await generateQRCodeWithLogo(String(customer.memberId || customer._id));
      if (qrDataUrl) {
        attachments.push({
    ...
    // Generate QR codes for prepaid passes as CID attachments
    for (let i = 0; i < prepaidPasses.length; i++) {
      const passQrDataUrl = await generateQRCodeWithLogo(prepaidPasses[i].code);
      if (passQrDataUrl) {
        attachments.push({
          filename: `prepaid-qr-${i + 1}.png`,
          content: passQrDataUrl.split(',')[1],
          encoding: 'base64',
          cid: `prepaid-qr-${i}`
        });
      }
    ...
    const content = `
          ...
          <td style="width: 33.33%; text-align: right; vertical-align: top;">
            ...
            <div style="font-size: 15px; font-weight: normal; color: #424770;">
              ${transaction.paymentMethod === 'card' || transaction.paymentMethod === 'stripe' ?
                ...
                transaction.paymentMethod === 'company' ? 'Company' :
                transaction.paymentMethod === 'invoice' || transaction.paymentMethod === 'customer-invoice' ? 'Invoice' :
                'Cash'}
            </div>
          </td>
        </tr>
      ...
      ${hasCustomerQR && attachments.find(a => a.cid === 'checkin-qr') ? `
      <!-- QR Code section -->
      <div style="margin-top: 40px; text-align: center; padding: 30px; background-color: #f6f9fc; border-radius: 6px;">
        ...
        <div style="display: inline-block;">
          <img src="cid:checkin-qr" alt="Check-in QR Code" style="max-width: 180px; height: auto; display: block;" />
        </div>
      ...
      ${prepaidPasses.length > 0 ? prepaidPasses.map((pass, i) => `
      ...
      <div style="margin-top: 40px; text-align: center; padding: 30px; background-color: #f6f9fc; border-radius: 6px;">
        ...
        <div style="display: inline-block;">
          <img src="cid:prepaid-qr-${i}" alt="Prepaid Pass QR Code" style="max-width: 180px; height: auto; display: block;" />
        </div>
        ...
        ${pass.expiresAt ? `
        <div style="color: #f59e0b; font-size: 13px; margin-top: 8px; font-weight: 500;">
          Use by: ${dayjs(pass.expiresAt).format('DD MMM YYYY')}
        </div>
        ` : ''}
      </div>`).join('') : ''}
    `;
...
Payment Method: ${transaction.paymentMethod === 'stripe' || transaction.paymentMethod === 'card' ? 'Card' : transaction.paymentMethod === 'company' ? 'Company' : transaction.paymentMethod === 'invoice' || transaction.paymentMethod === 'customer-invoice' ? 'Invoice' : 'Cash'}
