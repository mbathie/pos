...
/**
 * Check if an action is restricted for a user role
 * @param {string} userRole - The user's role (e.g., 'ADMIN', 'STAFF')
 * @param {string} action - The action to check (e.g., 'process_refund', 'custom_discount')
 * @returns {boolean} - True if action is restricted, false otherwise
 */
export function isActionRestricted(userRole, action) {
  if (!userRole || !action) {
    return true; // Assume restricted if missing parameters
  }

  const roleConfig = permissionsConfig.roles[userRole];
  if (!roleConfig) {
    return true; // Unknown role, assume restricted
  }

  // Admin and Manager have wildcard permissions
  if (roleConfig.permissions.includes('*')) {
    return false;
  }

  // Check if action is in restricted array
  const restricted = roleConfig.restricted || [];
