...
export async function calcCartValueCourse({ product }) {
  ...
  // Calculate remaining weeks for pro-rated pricing
  ...
  if (schedule?.startDate && schedule?.endDate) {
    ...
    totalWeeks = Math.max(1, Math.ceil((endDate - startDate) / msPerWeek));
    const effectiveStart = now > startDate ? now : startDate;
    remainingWeeks = Math.max(1, Math.ceil((endDate - effectiveStart) / msPerWeek));
  }

  const numSlots = product?.selectedSlots?.length || 1;

  // Price per slot per week × qty × slots × remaining weeks
  if (product?.prices?.length) {
    product.prices.forEach(price => {
      const qty = price.qty ?? 0;
      if (qty > 0) {
        amount.subtotal += qty * (parseFloat(price.value) || 0) * numSlots * remainingWeeks;
  ...
  amount.remainingWeeks = remainingWeeks;
  amount.totalWeeks = totalWeeks;
  amount.numSlots = numSlots;
...
export async function calcCartValuePrepaid({ product }) {
  const amount = {
    subtotal: 0,
    tax: 0,
    total: 0,
