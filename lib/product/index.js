...
export async function calcCartValueShop({product}) {
  ...
  // Add selected modifiers from modGroupsData - snapshot the full data
  if (product?.modGroupsData) {
    product.modGroupsData.forEach(group => {
      const selectedMods = group.mods?.filter(mod => mod.selected);
      if (selectedMods?.length) {
        selectedMods.forEach(mod => {
          amount.subtotal += parseFloat(mod.price) || 0;
          item.mods.push({
            ...mod,
            groupName: group.name
          });
        });
        // Store the entire group with selected mods for cart snapshot
        item.modGroups.push({
          _id: group._id,
          name: group.name,
